...
** Client or Server Side Search
   ...
   To do that, we can retool the search bar for server side search.  We're going to add a button to our search bar, and connect the search function to the =<form onSubmit={fn}=.

   HTML forms work a bit differently in React since we wanted a single source of state (see []
